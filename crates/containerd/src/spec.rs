...
pub fn build_oci_spec(data_dir: &DataDir, spec: &ContainerSpec) -> Result<Spec> {
    let env: Vec<String> = spec.env.iter().map(|(k, v)| format!("{k}={v}")).collect();

    let args = match spec.command {
        Some(ref cmd) => cmd.clone(),
        None => vec![], // empty args → containerd uses image ENTRYPOINT/CMD
    };
    ...
    // Use OCI spec version 1.1.0 so containerd uses cgroup v2 defaults and
    // does not inject a legacy cgroup v1 mount at /sys/fs/cgroup, which
    // fails on cgroup v2 kernels and breaks container initialization.
...
mod tests {
    ...
    fn test_cpu_quota_fraction() {
        let spec = build_oci_spec(&data_dir(), &test_spec());
        assert!(spec.is_ok());
        let spec = spec.ok();
        let cpu = spec
            .as_ref()
    ...
    fn test_full_cpu_quota() {
        let mut spec = test_spec();
        spec.cpu = 2.0;
        let oci = build_oci_spec(&data_dir(), &spec);
        assert!(oci.is_ok());
        let oci = oci.ok();
        let cpu = oci
            .as_ref()
    ...
    fn test_oci_spec_with_command() {
        ...
        let oci = build_oci_spec(&data_dir(), &spec).unwrap();
    ...
    #[test]
    fn test_oci_spec_without_command() {
        let spec = test_spec();
        assert!(spec.command.is_none());
        let oci = build_oci_spec(&data_dir(), &spec).unwrap();
        let args = oci.process().as_ref().and_then(|p| p.args().as_ref());
        // When command is None, args is empty — containerd uses image ENTRYPOINT/CMD
        assert_eq!(args.cloned().unwrap_or_default(), Vec::<String>::new());
    }
