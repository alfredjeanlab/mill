...
pub async fn init(
    ...
    let api_addr: SocketAddr = format!("{}:{API_PORT}", wg_identity.tunnel_ip)
        .parse()
        .unwrap_or_else(|_| SocketAddr::new(wg_identity.tunnel_ip, API_PORT));
    ...
    // 11. Create and run secondary, start RPC server
    ...
    let rpc_token = cluster_token.clone();
...
pub async fn join(
    ...
    // 4. Get cluster key over the WireGuard tunnel
    let leader_tunnel = join_info
        .peers
        .first()
        .map(|p| p.tunnel_ip)
        .ok_or_else(|| NodeError::NodeUnreachable("no peers in join-info".to_string()))?;
    let tunnel_api = client::DaemonClient::new(
        format!("http://{}:{}", leader_tunnel, join_info.api_port),
        token.to_string(),
    );
    let key_bytes: Vec<u8> = tunnel_api.get_bytes("/v1/cluster-key").await?.to_vec();
    ...
    // 8. Tell the leader to add us
    let resources = detect_resources();
    let our_api_addr: SocketAddr = format!("{}:{API_PORT}", tunnel_ip)
        .parse()
        .unwrap_or_else(|_| SocketAddr::new(tunnel_ip, API_PORT));
    ...
    // 10. Start RPC server
    let rpc_cancel = CancellationToken::new();
    let rpc_tok = token.to_string();
    tokio::spawn(async move {
        if let Err(e) = RpcServer::start(rpc_addr, rpc_tok, handle, rpc_cancel).await {
            tracing::error!("RPC server failed: {e}");
        }
    });
...
async fn start_from_existing(data_dir: &DataDir) -> Result<()> {
    ...
    let cancel = CancellationToken::new();
    let api_addr: SocketAddr = format!("{}:{}", wg_identity.tunnel_ip, API_PORT)
        .parse()
        .unwrap_or_else(|_| SocketAddr::new(wg_identity.tunnel_ip, API_PORT));
    let primary_config = PrimaryConfig {
        api_addr,
