...
/// A minimal service config for deploy: single service, 2 replicas to spread across nodes.
...
async fn drain_and_remove_node() {
    ...
    let deploy_output = api.deploy(SERVICE_CONFIG).await.expect("POST /v1/deploy");
    assert!(!deploy_output.is_empty(), "deploy should return SSE progress");

    // Wait for the service to become healthy with 2 replicas.
    poll_async(|| {
        let api = Api::new(node1.api_addr, TOKEN);
        async move {
    ...
    assert!(
        web.replicas.healthy >= 1,
        "service should have at least 1 healthy replica after drain"
    );

    // All remaining allocs for the service should be on node-1 (not node-2).
    for alloc in &web.allocations {
        assert_ne!(alloc.node, node2_id, "no allocations should remain on the drained node",);
    }
    ...
    let services = api.services().await.expect("GET /v1/services after remove");
    let web = services.iter().find(|s| s.name == "web").expect("service 'web' should still exist");
    assert!(web.replicas.healthy >= 1, "service should remain healthy after node removal",);
